...
- (void)requestTokenWithAuthGrant:(NSString *)authGrant redirectURL:(NSURL *)redirectURL;
{
    ...
    //Added to handle issue with ACN Auth system do not lose when updating
    NSString* redirectURLString = [redirectURL absoluteString];
    if ([redirectURLString hasSuffix:@"/"]) {
        redirectURLString = [redirectURLString substringToIndex:redirectURLString.length-1];
    }
...
{
    [self refreshAccessTokenAndRetryConnection:retryConnection withConentType:nil authorizationHeader:nil];
}

- (void)refreshAccessTokenAndRetryConnection:(NXOAuth2Connection *)retryConnection
                              withConentType:(NSString*)contentType
                         authorizationHeader:(NSString*)authHeader;
{
    if (retryConnection) {
        if (!waitingConnections) waitingConnections = [[NSMutableArray alloc] init];
        [waitingConnections addObject:retryConnection];
    ...
    if (!authConnection) {
        ...
        if (contentType != nil) {
            [tokenRequest setValue:contentType forHTTPHeaderField:@"Content-Type"];
        }
